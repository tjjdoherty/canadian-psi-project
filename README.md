...
## Jan 2025 Update:
This project will be developed in ways to illustrate an expanding Data Science skillset e.g. pipeline to automate and standardize preprocessing and cleaning of data. It seems unlikely at the moment that the data here could be used to make a supervised learning model in predicting school revenue or international enrolment itself as these are both impacted by a variety of nonlinear and intangible effects that aren't captured in the enrolment data alone which I have access to. We could use clustering in an unsupervised learning model, but with no clear target that task is not a priority.
